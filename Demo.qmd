...
#check what happens on the intercept a (a is in %??)
```
...
calculate_visits(a = 0, b = 100, SVD = 10, loss = 0.1, from_ = 0, to_ = 100, 
                             plot_ = TRUE, col_ = 2)
#we can calculate loss as % decrease from linea relationship
```
...
```{r}
#try with mikes data
calculate_visits(a = 3, b = 6, SVD = 2, loss = 0.6, from_ = 0, to_ = 5, 
                             plot_ = TRUE, col_ = 2)
#why a and the plotted intercept are not equal?
```
...
-   We can aproximate the loss to fit the b param autmatically.
