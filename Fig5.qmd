...
We can also predict how different pollinator communities might affect yield. For this we focus on Northern Highbush blueberry. We compare a mix of pollinators, vs honeybees dominated vs bumblebee dominated communities.

```{r}

#Note try better for SHB, as it can be nicer for thier pollinators.
...
NHBmix <- subset(NHighbushBlueberry, Pollinator == "mix")
NHBhb <- subset(NHighbushBlueberry, Pollinator == "Apis.mellifera")
NHBbb <- subset(NHighbushBlueberry, Pollinator == "Bombus.terrestris")

#calculate different SVD values
NHB_SVE_mix <- mean(subset(NHBmix, 
                       Number.of.visits == 2)$PollinationValue)/2
NHB_SVE_hb <- mean(subset(NHBhb, 
                       Number.of.visits == 1)$PollinationValue)
NHB_SVE_bb <- mean(subset(NHBbb, 
                       Number.of.visits == 1)$PollinationValue)


#calculate param c
NHB_c_mix <- calculate_visits(a = NHB_a, b = NHB_b, SVD = NHB_SVE_mix, to_ = 15)$c_parameter
NHB_c_hb <- calculate_visits(a = NHB_a, b = NHB_b, SVD = NHB_SVE_hb, to_ = 15)$c_parameter
NHB_c_bb <- calculate_visits(a = NHB_a, b = NHB_b, SVD = NHB_SVE_bb, to_ = 15)$c_parameter
#now we can plot all three
plot(NHighbushBlueberry$PollinationValue ~ jitter(NHighbushBlueberry$Number.of.visits), 
     las = 1, xlab = "Visits", ylab = "Fruit weigth", type = "n",
     ylim = c(0,4), xlim = c(0,15))
plot_visits(NHB_a, NHB_b, NHB_c_mix, to_ = 15, 
            add_ = TRUE, col = "#D55E00")
plot_visits(NHB_a, NHB_b, NHB_c_hb, to_ = 15, 
            add_ = TRUE, col = "#E69F00")
plot_visits(NHB_a, NHB_b, NHB_c_bb, to_ = 15, 
